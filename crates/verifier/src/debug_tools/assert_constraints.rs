...
pub fn assert_constraints(inputs: ProverInput) {
    ...
    // Base trace.
    let (claim, trace, lookup_data) = Claim::write_trace::<Poseidon31MerkleChannel>(inputs);
    let mut tree_builder = commitment_scheme.tree_builder();
    ...
    let mut dummy_channel = Poseidon31Channel::default();

...
//!
//! This module contains all the MIR generation tests organized by feature category.
//! Test data (.cm source files) are located in tests/test_data/

#![feature(let_chains)]

use insta::assert_snapshot;

// Import test harness from tests directory
include!("test_harness.rs");
...
macro_rules! mir_test {
    ($test_name:ident, $subdir:expr) => {
        ...
        fn $test_name() {
            ...
            let path = concat!("test_data/", $subdir, "/", stringify!($test_name), ".cm");
...
mir_test!(mutual_recursion, "functions");
...
mir_test!(dead_code, "optimizations");

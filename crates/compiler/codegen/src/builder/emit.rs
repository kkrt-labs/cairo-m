...
impl super::CasmBuilder {
    ...
    pub(super) fn emit_push(&mut self, instr: InstructionBuilder) {
        self.instructions.push(instr);
    }

    /// Generate a fresh label name using the builder's counter.
    pub(super) fn emit_new_label_name(&mut self, prefix: &str) -> String {
    ...
    pub(super) fn emit_add_label(&mut self, mut label: Label) {
        label.address = Some(self.instructions.len());
        self.labels.push(label);
    }

    /// Track that a frame region has been written.
    pub(super) fn emit_touch(&mut self, offset: i32, size: usize) {
        if offset >= 0 {

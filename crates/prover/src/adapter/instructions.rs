...
//TODO: QM31 reserved for instructions
//TODO: add prev value
#[derive(Debug, Default)]
pub struct StateData {
    pub state: VmState,
    pub memory_args: [(u32, QM31, u32, u32); 4], // (addr, val, prev_clock, clock)
}
...
impl Instructions {
    pub fn push_instr<I>(&mut self, mut memory: I, state: VmState, clock: u32, memory_cache: &mut MemoryCache)
    where
        I: Iterator<Item = MemEntry>,
    {
        ...
        match opcode_id {
            ...
            5 => { // store_double_deref_fp
                ...
                let mem3 = memory.next().unwrap();
                state_data.memory_args[3] = memory_cache.push(mem3, clock);
                self.states_by_opcodes.store_double_deref_fp.push(state_data);
            }
            6 => { // store_imm
                let mem1 = memory.next().unwrap();
                state_data.memory_args[1] = memory_cache.push(mem1, clock);
            ...
            19 => { // jmp_abs_double_deref_fp
                let mem1 = memory.next().unwrap();
                state_data.memory_args[1] = memory_cache.push(mem1, clock);
                let mem2 = memory.next().unwrap();
                state_data.memory_args[2] = memory_cache.push(mem2, clock);
                self.states_by_opcodes.jmp_abs_double_deref_fp.push(state_data);
            }
            20 => { // jmp_abs_imm
                self.states_by_opcodes.jmp_abs_imm.push(state_data);
            }
            21 => { // jmp_abs_mul_fp_fp
                let mem1 = memory.next().unwrap();
                state_data.memory_args[1] = memory_cache.push(mem1, clock);
                let mem2 = memory.next().unwrap();
                state_data.memory_args[2] = memory_cache.push(mem2, clock);
            ...
            26 => { // jmp_rel_double_deref_fp
                let mem1 = memory.next().unwrap();
                state_data.memory_args[1] = memory_cache.push(mem1, clock);
                let mem2 = memory.next().unwrap();
                state_data.memory_args[2] = memory_cache.push(mem2, clock);
                self.states_by_opcodes.jmp_rel_double_deref_fp.push(state_data);
            }
            27 => { // jmp_rel_imm
                self.states_by_opcodes.jmp_rel_imm.push(state_data);
            }
            28 => { // jmp_rel_mul_fp_fp
                let mem1 = memory.next().unwrap();
                state_data.memory_args[1] = memory_cache.push(mem1, clock);
                let mem2 = memory.next().unwrap();
                state_data.memory_args[2] = memory_cache.push(mem2, clock);

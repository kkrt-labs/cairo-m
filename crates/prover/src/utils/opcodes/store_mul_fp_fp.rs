...
/// Packed structure for StoreMulFpFp opcode containing only the fields it uses
...
pub struct PackedExecutionBundle {
    ...
    pub dst_prev_clock: PackedM31,
    pub dst_mult: PackedM31,
    pub dst_val: PackedM31,
}
...
impl PackedExecutionBundle {
    ...
    pub fn pack_from(bundles: [ExecutionBundle; N_LANES]) -> Self {
        Self {
            ...
            dst_prev_clock: PackedM31::from_array(std::array::from_fn(|i| {
                bundles[i].operands[2].map_or_else(M31::zero, |op| op.prev_clock)
            })),
            dst_mult: PackedM31::from_array(std::array::from_fn(|i| {
                bundles[i].operands[2].map_or_else(M31::zero, |op| op.multiplicity)
            })),
            dst_val: PackedM31::from_array(std::array::from_fn(|i| {

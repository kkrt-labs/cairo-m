...
pub const LOG_SIZE_RC_16: u32 = 16;

pub struct InteractionClaimData {
    pub range_check_16: Vec<[PackedM31; 2]>,
}
...
impl Claim {
    ...
    /// Writes the preprocessed range_check_16 trace
    ///
    ...
    {
        let mults_atomic: Vec<AtomicU32> = (0..1 << LOG_SIZE_RC_16)
        ...
        let domain = CanonicCoset::new(LOG_SIZE_RC_16).circle_domain();
        (
            Self {
                log_size: LOG_SIZE_RC_16,
            },
...
impl InteractionClaim {
    ...
    pub fn write_interaction_trace(
        range_check_16: &RangeCheck16,
        interaction_claim_data: &InteractionClaimData,
    ) -> (
        Self,
        impl IntoIterator<Item = CircleEvaluation<SimdBackend, BaseField, BitReversedOrder>>,
    ) {
        let log_size = interaction_claim_data.range_check_16.len().ilog2() + LOG_N_LANES;
        let mut interaction_trace = LogupTraceGenerator::new(log_size);

        let mut col = interaction_trace.new_col();
        (col.par_iter_mut(), &interaction_claim_data.range_check_16)
            .into_par_iter()
            .for_each(|(writer, value)| {
                let denom: PackedQM31 = range_check_16.combine(&[value[0]]);
...
pub struct Eval {
    pub claim: Claim,
    pub relation: RangeCheck16,
}
impl Eval {
    pub const fn new(claim: Claim, relation: RangeCheck16) -> Self {
        Self { claim, relation }
    }
}
...
impl FrameworkEval for Eval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        let value = eval.get_preprocessed_column(RangeCheck::new(LOG_SIZE_RC_16).id());

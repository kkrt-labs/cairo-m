...
pub const LOG_SIZE_RC_8: u32 = 8;

pub struct InteractionClaimData {
    pub range_check_8: Vec<[PackedM31; 2]>,
}
...
impl Claim {
    ...
    /// Writes the preprocessed range_check_8 trace
    ///
    /// lookup_data contains all range_checks made in other components during main trace generation
    /// write_trace creates a column with all values from 0 to 2**8 - 1 included and counts how many times other components
        ...
        SimdBackend: BackendForChannel<MC>,
    {
        let mults_atomic: Vec<AtomicU32> =
            (0..1 << LOG_SIZE_RC_8).map(|_| AtomicU32::new(0)).collect();
        ...
        let domain = CanonicCoset::new(LOG_SIZE_RC_8).circle_domain();
        (
            Self {
                log_size: LOG_SIZE_RC_8,
            },
...
impl InteractionClaim {
    ...
    pub fn write_interaction_trace(
        range_check_8: &RangeCheck8,
        interaction_claim_data: &InteractionClaimData,
    ) -> (
        Self,
        impl IntoIterator<Item = CircleEvaluation<SimdBackend, BaseField, BitReversedOrder>>,
    ) {
        let log_size = interaction_claim_data.range_check_8.len().ilog2() + LOG_N_LANES;
        let mut interaction_trace = LogupTraceGenerator::new(log_size);

        let mut col = interaction_trace.new_col();
        (col.par_iter_mut(), &interaction_claim_data.range_check_8)
            .into_par_iter()
            .for_each(|(writer, value)| {
                let denom: PackedQM31 = range_check_8.combine(&[value[0]]);
...
pub struct Eval {
    pub claim: Claim,
    pub relation: RangeCheck8,
}
impl Eval {
    pub const fn new(claim: Claim, relation: RangeCheck8) -> Self {
        Self { claim, relation }
    }
}
...
impl FrameworkEval for Eval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        let value = eval.get_preprocessed_column(RangeCheck::new(LOG_SIZE_RC_8).id());

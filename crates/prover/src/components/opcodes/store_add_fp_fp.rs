...
//!   * `opcode_id - 0`
//! * read op0
//!   * `- [fp + off0, clk, op0_val]` in `Memory` relation
//!   * `- [clk - op0_prev_clk - 1]` in `RangeCheck_20` relation
//! * read op1
//!   * `- [fp + off1, clk, op1_val]` in `Memory` relation
//!   * `- [clk - op1_prev_clk - 1]` in `RangeCheck_20` relation
//! * write dst in [fp + off2]
//!   * `+ [fp + off2, clk, op0_val + op1_val]` in `Memory` relation
...
use crate::utils::opcodes::store_add_fp_fp::PackedExecutionBundle;
        ...
        SimdBackend: BackendForChannel<MC>,
    {
        ...
        (
            ...
            .for_each(|(row_index, (mut row, input, lookup_data))| {
                ...
                let op1_prev_clock = input.op1_prev_clock;
                let op1_val = input.op1_val;
                let dst_mult = input.dst_mult;
    ...
    pub fn write_interaction_trace(
        ...
        (
            ...
            .for_each(|(i, (writer, memory2, memory3))| {
                // memory2 is op1 read, memory3 is dst write with multiplicity
                let num2 = -PackedQM31::from(enabler_col.packed_at(i));
                let denom2: PackedQM31 = memory_relation.combine(&memory2[..6]);
                let num3 = PackedQM31::from(enabler_col.packed_at(i) * memory3[6]);
...
impl FrameworkEval for Eval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let dst_multiplicity = eval.next_trace_mask();

        // Enabler is 1 or 0
        eval.add_constraint(enabler.clone() * (one.clone() - enabler.clone()));

        // Opcode id is StoreAddFpFp
        ...
        // Read op1 - current state only
        eval.add_to_relation(RelationEntry::new(
            &self.memory,
            -E::EF::from(enabler.clone()),
            &[fp.clone() + off1, op1_prev_clock.clone(), op1_val.clone()],
        ));

        // Write dst
        eval.add_to_relation(RelationEntry::new(
            &self.memory,
            E::EF::from(enabler.clone() * dst_multiplicity),

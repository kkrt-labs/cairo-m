...
//! * read [base + offset]
//!   * `- [base_val + offset_val, final_deref_prev_clock, final_deref_val] + [base_val + offset_val, clk, final_deref_val]` in `Memory` relation
...
use cairo_m_common::instruction::STORE_DOUBLE_DEREF_FP_FP;
...
impl Claim {
    ...
    /// Writes the trace for the StoreDoubleDerefFpFp opcode.
    ///
    /// # Important
        ...
        SimdBackend: BackendForChannel<MC>,
    {
        ...
        (
            ...
            .for_each(|(row_index, (mut row, input, lookup_data))| {
                ...
                let opcode_constant = PackedM31::from(M31::from(STORE_DOUBLE_DEREF_FP_FP));
                let base_off = input.inst_value_1;
                let offset_off = input.inst_value_2;
                let dst_off = input.inst_value_3;

                // Read base value
                let base_prev_clock = get_prev_clock(input, data_accesses, 0);
                let base_val = get_value(input, data_accesses, 0);

                // Read offset value
                let offset_prev_clock = get_prev_clock(input, data_accesses, 1);
                let offset_val = get_value(input, data_accesses, 1);

                // Read final dereferenced value
                let final_deref_prev_clock = get_prev_clock(input, data_accesses, 2);
                let final_deref_val = get_value(input, data_accesses, 2);

                // Get destination previous values
                let dst_prev_val = get_prev_value(input, data_accesses, 3);
                let dst_prev_clock = get_prev_clock(input, data_accesses, 3);

                // Write trace columns in the order specified in the AIR
                *row[0] = enabler;
                *row[1] = pc;
                *row[2] = fp;
                *row[3] = clock;
                ...
                *row[7] = dst_off;
                *row[8] = base_val;
                *row[9] = base_prev_clock;
                *row[10] = offset_val;
                *row[11] = offset_prev_clock;
                *row[12] = final_deref_val;
                *row[13] = final_deref_prev_clock;
                *row[14] = dst_prev_val;
                *row[15] = dst_prev_clock;
                ...
                *lookup_data.memory[3] = [fp + base_off, clock, base_val, zero, zero, zero];
                ...
                *lookup_data.memory[5] = [fp + offset_off, clock, offset_val, zero, zero, zero];
                ...
                *lookup_data.memory[9] = [fp + dst_off, clock, final_deref_val, zero, zero, zero];

                // Range checks
                *lookup_data.range_check_20[0] = clock - inst_prev_clock - enabler;
                *lookup_data.range_check_20[1] = clock - base_prev_clock - enabler;
                *lookup_data.range_check_20[2] = clock - offset_prev_clock - enabler;
                *lookup_data.range_check_20[3] = clock - final_deref_prev_clock - enabler;
                *lookup_data.range_check_20[4] = clock - dst_prev_clock - enabler;
            });
...
impl InteractionClaim {
    ...
    pub fn write_interaction_trace(
        ...
        (
            ...
            .into_par_iter()
            .for_each(|(writer, range_check_20)| {
                let num = -PackedQM31::one();
                let denom: PackedQM31 = relations.range_check_20.combine(&[*range_check_20]);
...
impl FrameworkEval for Eval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let opcode_constant = E::F::from(M31::from(STORE_DOUBLE_DEREF_FP_FP));
        ...
        let final_deref_val = eval.next_trace_mask();
        ...
        eval.add_to_relation(RelationEntry::new(
            &self.relations.memory,
            E::EF::from(enabler.clone()),
            &[fp.clone() + offset_off, clock.clone(), offset_val.clone()],
        ));
        ...
        eval.add_to_relation(RelationEntry::new(
            &self.relations.memory,
            E::EF::from(enabler.clone()),
            &[fp + dst_off, clock.clone(), final_deref_val],
        ));
        ...
        eval.add_to_relation(RelationEntry::new(
            &self.relations.range_check_20,
            -E::EF::one(),
            &[clock.clone() - final_deref_prev_clock - enabler.clone()],
        ));
        eval.add_to_relation(RelationEntry::new(
            &self.relations.range_check_20,
            -E::EF::one(),

//! This component is used to prove the JmpRelAddFpFp opcode.
...
//!   * `- [pc, fp] + [pc + op0_val + op1_val, fp]` in `Registers` relation
//! * read instruction from memory
//!   * `- [pc, inst_prev_clk, opcode_id, off0, off1, off2]` in `Memory` relation
//!   * `- [clk - inst_prev_clk - 1]` in `RangeCheck_20` relation
//! * assert opcode id
//!   * `opcode_id - 23`
//! * read op0
//!   * `- [fp + off0, op0_prev_clk, op0_val]` in `Memory` relation
//!   * `- [clk - op0_prev_clk - 1]` in `RangeCheck_20` relation
//! * read op1
//!   * `- [fp + off1, op1_prev_clk, op1_val]` in `Memory` relation
...
impl Claim {
    ...
    /// Writes the trace for the JmpRelAddFpFp opcode.
    ///
    /// # Important
        ...
        SimdBackend: BackendForChannel<MC>,
    {
        ...
        (
            ...
            .for_each(|(row_index, (mut row, input, lookup_data))| {
                ...
                *lookup_data.registers[1] = [input.pc + op0_val + op1_val, input.fp];
                ...
                *lookup_data.memory[1] =
                ...
                // Read op1 - single lookup with multiplicity 1
...
impl FrameworkEval for Eval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let expected_opcode_id = E::F::from(M31::from(Opcode::JmpRelAddFpFp));
        ...
        eval.add_to_relation(RelationEntry::new(
            &self.registers,
            E::EF::from(enabler.clone()),
            &[pc.clone() + op0_val.clone() + op1_val.clone(), fp.clone()],
        ));

//! This component is used to prove the StoreDerefFp opcode.
//! [fp + off2] = [fp + off0]
//!
//! # Columns
//!
...
//! - dst_prev_val
//! - dst_prev_clock
//! - mult (memory write multiplicity)
//!
//! # Constraints
//!
//! * enabler is a bool
...
//!   [pc, clock, opcode_id, off0, off1, off2] in Memory relation with multiplicity enabler
//!   - [clock - inst_prev_clock - 1] in RangeCheck_20 relation
//!
//! * assert opcode id
//!   opcode_id - 3
//!
//! * read op0
//!   [fp + off0, clock, op0_val] in Memory relation with multiplicity enabler
//!   - [clock - op0_prev_clock - 1] in RangeCheck_20 relation
//!
//! * write dst in [fp + off2]
//!   [fp + off2, clock, op0_val] in Memory Relation with multiplicity enabler*mult
//!   - [clock - dst_prev_clock - 1] in RangeCheck_20 relation
...
impl Claim {
    ...
    /// Writes the trace for the StoreDerefFp opcode.
    ///
    /// # Important
        ...
        (
            ...
            .for_each(|(row_index, (mut row, input, lookup_data))| {
                ...
                *lookup_data.memory[0] =
                    [input.pc, inst_prev_clock, opcode_id, off0, off1, off2, zero];
                *lookup_data.memory[1] =
                    [fp + off0, op0_prev_clock, op0_val, zero, zero, zero, zero];
                *lookup_data.memory[2] = [fp + off2, clock, op0_prev_val, zero, zero, zero, mult];
    ...
    pub fn write_interaction_trace(
        ...
        (
            ...
            .for_each(|(i, (writer, memory0, memory1))| {
                // memory0 is instruction read, memory1 is op0 read
                let enabler = enabler_col.packed_at(i);
                let num0 = -PackedQM31::from(enabler);
                let denom0: PackedQM31 = memory_relation.combine(&memory0[..6]);
                let num1 = -PackedQM31::from(enabler);
        ...
        (
            ...
            .for_each(|(i, (writer, memory2, range_check_20_0))| {
                // memory2 is dst write with multiplicity
                let enabler = enabler_col.packed_at(i);
                let num = PackedQM31::from(enabler * memory2[6]);
                let denom: PackedQM31 = memory_relation.combine(&memory2[..6]);
                let num_rc = -PackedQM31::one();
                let denom_rc: PackedQM31 = range_check_20_relation.combine(&[*range_check_20_0]);

                let numerator = num * denom_rc + num_rc * denom;
                let denom = denom * denom_rc;

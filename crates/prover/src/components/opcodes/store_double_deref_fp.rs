...
//! * read [op0 + off1]
//!   * `- [op0_val + off1, op0_deref_prev_clk, op0_deref_val] + [op0_val + off1, clk, op0_deref_val]` in `Memory` relation
//!   * `- [clk - op0_deref_prev_clk - 1]` in `RangeCheck20` relation
//! * write dst in [fp + off2]
//!   * `- [fp + off2, dst_prev_clk, dst_prev_val] + [fp + off2, clk, op0_deref_val]` in `Memory` Relation
...
impl Claim {
        ...
        SimdBackend: BackendForChannel<MC>,
    {
        ...
        let m31_instruction_size = PackedM31::from(M31::from(
            Instruction::size_in_m31s_for_opcode(STORE_DOUBLE_DEREF_FP).unwrap(),
        ));
        ...
        (
            ...
            .for_each(|(row_index, (mut row, input, lookup_data))| {
                ...
                *lookup_data.memory[4] = [input.pc + one + one, inst_prev_clock, off1];
                *lookup_data.memory[5] = [input.pc + one + one, clock, off1];
                *lookup_data.memory[6] = [input.pc + one + one + one, inst_prev_clock, off2];
                *lookup_data.memory[7] = [input.pc + one + one + one, clock, off2];

                *lookup_data.memory[8] = [fp + off0, op0_prev_clock, op0_val];
                *lookup_data.memory[9] = [fp + off0, clock, op0_val];

                *lookup_data.memory[10] = [op0_val + off1, op0_deref_prev_clock, op0_deref_val];
                *lookup_data.memory[11] = [op0_val + off1, clock, op0_deref_val];

                *lookup_data.memory[12] = [fp + off2, dst_prev_clock, dst_prev_val];
                *lookup_data.memory[13] = [fp + off2, clock, op0_deref_val];
...
impl InteractionClaim {
    ...
    pub fn write_interaction_trace(
        ...
        // Instruction memory lookups - off1
        let mut col = interaction_trace.new_col();
        (
            col.par_iter_mut(),
        ...
        // Instruction memory lookups - off2
        let mut col = interaction_trace.new_col();
        (
            col.par_iter_mut(),
...
impl FrameworkEval for Eval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        eval.add_to_relation(RelationEntry::new(
            &self.relations.memory,
            E::EF::from(enabler.clone()),
            &[pc.clone() + one.clone(), clock.clone(), off0.clone()],
        ));
        eval.add_to_relation(RelationEntry::new(
            &self.relations.memory,
            -E::EF::from(enabler.clone()),
        ...
        eval.add_to_relation(RelationEntry::new(
            ...
            -E::EF::from(enabler.clone()),
            &[
                pc.clone() + one.clone() + one.clone() + one.clone(),
                inst_prev_clock.clone(),
                off2.clone(),
            ],
        ));

...
//! * read [base + imm]
//!   * `- [base_val + imm, deref_prev_clk, deref_val] + [base_val + imm, clk, deref_val]` in `Memory` relation
//!   * `- [clk - deref_prev_clk - 1]` in `RangeCheck20` relation
//! * write dst in [fp + dst_off]
//!   * `- [fp + dst_off, dst_prev_clk, dst_prev_val] + [fp + dst_off, clk, deref_val]` in `Memory` Relation
        ...
        SimdBackend: BackendForChannel<MC>,
    {
        ...
        (
            ...
            .for_each(|(row_index, (mut row, input, lookup_data))| {
                ...
                let deref_prev_clock = get_prev_clock(input, data_accesses, 1);
                let deref_val = get_value(input, data_accesses, 1);
                ...
                *lookup_data.memory[1] = [input.pc, clock, opcode_constant, base_off, imm, dst_off];
...
impl FrameworkEval for Eval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let deref_val = eval.next_trace_mask();
        let deref_prev_clock = eval.next_trace_mask();
        ...
        // Read [base + imm] (double dereference)
        eval.add_to_relation(RelationEntry::new(
            &self.relations.memory,
            -E::EF::from(enabler.clone()),
            &[
                base_val.clone() + imm.clone(),
                deref_prev_clock.clone(),
                deref_val.clone(),
            ],
        ));
        eval.add_to_relation(RelationEntry::new(
            &self.relations.memory,
            E::EF::from(enabler.clone()),
            &[base_val + imm, clock.clone(), deref_val.clone()],
        ));
        ...
        eval.add_to_relation(RelationEntry::new(
            &self.relations.memory,
            E::EF::from(enabler.clone()),
            &[fp + dst_off, clock.clone(), deref_val],
        ));
        ...
        eval.add_to_relation(RelationEntry::new(
            &self.relations.range_check_20,
            -E::EF::one(),
            &[clock.clone() - deref_prev_clock - enabler.clone()],
        ));
        eval.add_to_relation(RelationEntry::new(
            &self.relations.range_check_20,
            -E::EF::one(),

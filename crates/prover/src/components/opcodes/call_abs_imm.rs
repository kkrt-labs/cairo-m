//! This component is used to prove the CallAbsImm opcode.
//! call abs imm
//!
//! # Columns
//!
...
//!   * `- [pc, inst_prev_clk, off0] + [pc, clk, off0]` in `Memory` relation
//!   * `- [pc, inst_prev_clk, off1] + [pc, clk, off1]` in `Memory` relation
...
//!   * `- [clk - op0_plus_one_prev_clk - 1]` in `RangeCheck20` relation
...
impl Claim {
    ...
    /// Writes the trace for the CallAbsImm opcode.
    ///
    /// # Important
        ...
        SimdBackend: BackendForChannel<MC>,
    {
        ...
        let m31_instruction_size = PackedM31::from(M31::from(
            Instruction::size_in_m31s_for_opcode(CALL_ABS_IMM).unwrap(),
        ));
        ...
        (
            ...
            .for_each(|(row_index, (mut row, input, lookup_data))| {
                ...
                let opcode_constant = PackedM31::from(M31::from(CALL_ABS_IMM));
                let off0 = input.inst_value_1;
                let off1 = input.inst_value_2;
                let op0_prev_clock = input.mem1_prev_clock;
                let op0_prev_val = input.mem1_prev_value_limb0;
                let op0_plus_one_prev_clock = input.mem2_prev_clock;
                let op0_plus_one_prev_val = input.mem2_prev_value_limb0;
                ...
                *lookup_data.memory[6] = [fp + off0, op0_prev_clock, op0_prev_val];
                *lookup_data.memory[7] = [fp + off0, clock, fp];
                ...
                *lookup_data.memory[9] = [fp + off0 + one, clock, pc + m31_instruction_size];
...
impl FrameworkEval for Eval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        eval.add_to_relation(RelationEntry::new(
            &self.relations.memory,
            E::EF::from(enabler.clone()),
            &[fp + off0 + one, clock.clone(), pc + m31_instruction_size],
        ));
        ...
        eval.add_to_relation(RelationEntry::new(
            &self.relations.range_check_20,
            -E::EF::one(),
            &[clock - op0_plus_one_prev_clock - enabler],
        ));

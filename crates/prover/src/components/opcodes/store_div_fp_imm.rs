//! This component is used to prove the StoreDivFpImm opcode.
...
use crate::utils::opcodes::store_div_fp_imm::PackedExecutionBundle;
...
impl InteractionClaim {
    ...
    pub fn write_interaction_trace(
        ...
        (
            ...
            .into_par_iter()
            .enumerate()
            .for_each(|(i, (writer, memory2, range_check))| {
                // memory2 is dst write
                let num_mem = PackedQM31::from(enabler_col.packed_at(i) * memory2[6]);
                let denom_mem: PackedQM31 = memory_relation.combine(&memory2[..6]);
                let num_range = -PackedQM31::one();
                let denom_range: PackedQM31 = range_check_20_relation.combine(&[*range_check]);

                let numerator = num_mem * denom_range + num_range * denom_mem;
                let denom = denom_mem * denom_range;
...
impl FrameworkEval for Eval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let expected_opcode_id = E::F::from(M31::from(Opcode::StoreDivFpImm));
        ...
        // Opcode id is StoreDivFpImm
        eval.add_constraint(enabler.clone() * (opcode_id.clone() - expected_opcode_id));

        // Division constraint: dst_val * off1 - op0_val = 0
        eval.add_constraint(enabler.clone() * (dst_val.clone() * off1.clone() - op0_val.clone()));

//! This component is used to prove the CallRelFp opcode.
//! call rel [fp + off1]
//!
//! # Columns
//!
...
//!   * `- [pc, fp] + [pc + op1_val, fp + off0 + 2]` in `Registers` relation
...
//!   * `opcode_id - 13`
//! * write return fp
//!   * `- [fp + off0, op0_prev_clk, op0_val] + [fp + off0, clk, fp]` in `Memory` relation
        ...
        SimdBackend: BackendForChannel<MC>,
    {
        ...
        (
            ...
            .for_each(|(row_index, (mut row, input, lookup_data))| {
                ...
                *lookup_data.registers[1] = [pc + op1_val, input.fp + off0 + one + one];
                ...
                *lookup_data.range_check_20[2] = clock - op0_plus_one_prev_clock - one;
                *lookup_data.range_check_20[3] = clock - op1_prev_clock - one;
            });
...
impl FrameworkEval for Eval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let expected_opcode_id = E::F::from(M31::from(Opcode::CallRelFp));
        ...
        eval.add_to_relation(RelationEntry::new(
            &self.memory,
            E::EF::from(enabler.clone()),
            &[fp.clone() + off1.clone(), clock.clone(), op1_val.clone()],
        ));
        ...
        eval.add_to_relation(RelationEntry::new(
            &self.range_check_20,
            -E::EF::one(),
            &[clock.clone() - op0_plus_one_prev_clock.clone() - one.clone()],
        ));
        eval.add_to_relation(RelationEntry::new(
            &self.range_check_20,
            -E::EF::one(),
            &[clock.clone() - op1_prev_clock.clone() - one.clone()],
        ));

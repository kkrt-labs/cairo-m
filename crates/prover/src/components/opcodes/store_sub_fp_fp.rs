//! This component is used to prove the StoreSubFpFp opcode.
...
//!   * `- [fp + off2, dst_prev_clk, dst_prev_val] + [fp + off2, clk, op0 - op1]` in `Memory` relation
//!   * `- [clk - dst_prev_clk - 1]` in `RangeCheck20` relation

use cairo_m_common::instruction::STORE_SUB_FP_FP;
...
impl Claim {
    ...
    /// Writes the trace for the StoreSubFpFp opcode.
    ///
    /// # Important
        ...
        SimdBackend: BackendForChannel<MC>,
    {
        ...
        (
            ...
            .for_each(|(row_index, (mut row, input, lookup_data))| {
                ...
                let opcode_constant = PackedM31::from(M31::from(STORE_SUB_FP_FP));
                ...
                *lookup_data.memory[13] = [fp + off2, clock, op0_val - op1_val];
...
impl FrameworkEval for Eval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let opcode_constant = E::F::from(M31::from(STORE_SUB_FP_FP));
        ...
        eval.add_to_relation(RelationEntry::new(
            &self.relations.memory,
            E::EF::from(enabler.clone()),
            &[fp.clone() + off1, clock.clone(), op1_val.clone()],
        ));
        ...
        eval.add_to_relation(RelationEntry::new(
            &self.relations.memory,
            E::EF::from(enabler.clone()),
            &[fp + off2, clock.clone(), op0_val - op1_val],
        ));

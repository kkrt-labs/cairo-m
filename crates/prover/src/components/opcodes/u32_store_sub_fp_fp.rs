//! This component is used to prove the U32StoreSubFpFp opcode.
...
//! * read op1
//!   * `- [fp + src1_off, op1_prev_clk_lo, op1_val_lo] + [fp + src1_off, clk, op1_val_lo]`
...
use cairo_m_common::instruction::U32_STORE_SUB_FP_FP;
...
impl Claim {
    ...
    /// Writes the trace for the U32StoreSubFpFp opcode.
    ///
    /// # Important
        ...
        SimdBackend: BackendForChannel<MC>,
    {
        ...
        (
            ...
            .for_each(|(row_index, (mut row, input, lookup_data))| {
                ...
                let opcode_constant = PackedM31::from(M31::from(U32_STORE_SUB_FP_FP));
                ...
                let borrow_hi = PackedM31::from_array(
                    op0_val_hi
                        .to_array()
                        .iter()
                        .zip((op1_val_hi + borrow_lo).to_array().iter())
                        .map(|(x, y)| if x.0 < y.0 { M31::one() } else { M31::zero() })
                        .collect::<Vec<_>>()
                        .try_into()
                        .unwrap(),
                ...
                let res_lo = op0_val_lo + borrow_lo * two_pow_16 - op1_val_lo;
                let res_hi = op0_val_hi - borrow_lo + borrow_hi * two_pow_16 - op1_val_hi;
...
impl InteractionClaim {
    ...
    pub fn write_interaction_trace(
        ...
        // Range check 20 lookups (7 total, so 3 pairs + 1 single)
        ...
        // Handle the last odd range check 20 lookup (index 6)
        let mut col = interaction_trace.new_col();
        (
            col.par_iter_mut(),
            ...
            .for_each(|(_i, (writer, range_check_20_6))| {
                let num = -PackedQM31::one();
                let denom_0: PackedQM31 = relations.range_check_20.combine(&[*range_check_20_6]);
...
impl FrameworkEval for Eval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let opcode_constant = E::F::from(M31::from(U32_STORE_SUB_FP_FP));
        ...
        let res_lo =
            op0_val_lo.clone() + borrow_lo.clone() * two_pow_16.clone() - op1_val_lo.clone();
        let res_hi = op0_val_hi.clone() - borrow_lo.clone() + borrow_hi.clone() * two_pow_16
            - op1_val_hi.clone();
        ...
        // Borrow_lo is 1 or 0
        eval.add_constraint(borrow_lo.clone() * (one.clone() - borrow_lo));

        // Borrow_hi is 1 or 0
        eval.add_constraint(borrow_hi.clone() * (one.clone() - borrow_hi));

...
fn check_cargo_toml(
    ...
    if old_bins != new_bins {
        ...
        bail!(
            "The file `Cargo.toml` is outdated. Run `CairoMlings dev update` to update it. Then run `CairoMlings dev check` again"
        );
    }
...
fn check_exercises(info_file: &'static InfoFile, cmd_runner: &'static CmdRunner) -> Result<()> {
    match info_file.format_version.cmp(&CURRENT_FORMAT_VERSION) {
        ...
        Ordering::Greater => bail!(
            "`format_version` > {CURRENT_FORMAT_VERSION} (supported version)\n\
             Try updating the CairoMlings program"
        ),
        Ordering::Equal => (),
    }
...
pub fn check(require_solutions: bool) -> Result<()> {
    ...
    if cfg!(debug_assertions) {
        // A hack to make `cargo dev check` work when developing CairoMlings.

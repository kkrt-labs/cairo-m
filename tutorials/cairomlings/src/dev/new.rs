...
# CairoMlings program.
# The format version is not the same as the version of the CairoMlings program.
# In case CairoMlings makes an unavoidable breaking change to the expected format of community
# exercises, you would need to raise this version and adapt to the new format.
# Otherwise, the newest version of the CairoMlings program won't be able to run these exercises.
format_version = ";

const INFO_FILE_AFTER_FORMAT_VERSION: &str = r#"

# Optional multi-line message to be shown to users when just starting with the exercises.
welcome_message = """Welcome to these community CairoMlings exercises."""
...
# CairoMlings will always run Clippy on exercises.
...
const CARGO_TOML: &[u8] =
    br#"# Don't edit the `bin` list manually! It is updated by `CairoMlings dev update`
bin = []

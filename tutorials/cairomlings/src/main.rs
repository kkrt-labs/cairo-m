...
struct Args {
    ...
    /// Only use this if CairoMlings fails to detect exercise file changes.
    #[arg(long)]
    manual_run: bool,
}
...
#[derive(Subcommand)]
enum Subcommands {
    /// Initialize the official CairoMlings exercises
    Init,
    ...
    /// Commands for developing (community) CairoMlings exercises
    #[command(subcommand)]
    Dev(DevCommands),
}
...
fn main() -> Result<ExitCode> {
    ...
    if cfg!(not(debug_assertions)) && Path::new("dev/CairoMlings-repo.txt").exists() {
        bail!("{OLD_METHOD_ERR}");
    }
...
It is possible that you have an outdated version of CairoMlings.
Try to install the latest CairoMlings version first.";
...
If you are just starting with CairoMlings, run the command `CairoMlings init` to initialize it.";
